...
import Map from '../../../../map';
import findComplexTaxiPoints from './complex-taxi';
...
BRp.findComplexTaxiPoints = findComplexTaxiPoints;

BRp.findSimplePoints = function (edge, pairInfo) {
    const rs = edge._private.rscratch;
    rs.edgeType = 'segments';
    rs.segpts = [
        pairInfo.tgtPos.x,
        is.number(rs.startY) ? rs.startY : (-pairInfo.srcH / 2) + pairInfo.srcPos.y // (-pairInfo.srcH / 2) + pairInfo.srcPos.y,
    ];
};
